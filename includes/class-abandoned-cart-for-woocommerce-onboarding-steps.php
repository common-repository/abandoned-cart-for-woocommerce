...
class Abandoned_Cart_For_Woocommerce_Onboarding_Steps {
	...
	public function __construct() {
		...
		self::$mwb_acfw_plugin_name_label = 'Abandoned Cart for WooCommerce';
	...
	public function mwb_acfw_send_onboarding_data() {
		...
		try {
			...
			echo esc_html( wp_json_encode( $e->getMessage() ) );
	...
	private function mwb_acfw_hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			$status_code = 500;
			$response    = esc_html__( 'Unexpected Error Occured', 'abandoned-cart-for-woocommerce' );
